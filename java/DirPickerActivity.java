...
public class DirPickerActivity extends AppCompatActivity {
  ...
  private final String DEFVALUE_DIR_PICKER_LAST_PATH = "/";
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (path == null)
      path = PreferenceManager.getDefaultSharedPreferences(getApplicationContext())
        .getString(KEY_DIR_PICKER_LAST_PATH, DEFVALUE_DIR_PICKER_LAST_PATH);
    updateListDir();
  ...
  private void updateListDir () {
    ...
    for (File file : files) {
      ...
      if (isDir && dirOpened(file.getAbsolutePath()))
        arrayDir.add(m);
      else if (!isDir)
        arrayFiles.add(m);
    }
  ...
  private boolean dirOpened(String dirName) {
    try {
